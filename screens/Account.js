...
import AccountQuestion from "../components/data/accData";
...
const Account = () => {
  ...
  const [selectedAnswers, setSelectedAnswers] = useState(
    Array(AccountQuestion.length).fill(null)
  );

  const handleAnswers = (selectedAns) => {
    const answer = AccountQuestion[presentQuest]?.answer;
    ...
    if (nextQuestion < AccountQuestion.length) {
  ...
  const totalQuestions = AccountQuestion.length;
    ...
    <View style={styles.container}>
      {showScore ? (
        <ScrollView>
          <View style={styles.score__container}>
            ...
            <View style={styles.questionsContainer}>
              {AccountQuestion.map((item, index) => (
        ...
        <View>
          ...
          <Text style={styles.screen__title}>AccountQuestion Quiz</Text>
          <Text style={styles.questionNumber}>
            Question {presentQuest + 1} of {totalQuestions}
          </Text>
          <QuestionCard content={AccountQuestion[presentQuest]?.question} />
          <View style={styles.quest__opts}>
            {AccountQuestion[presentQuest]?.options.map((option) => {
              return (
                <TouchableOpacity
