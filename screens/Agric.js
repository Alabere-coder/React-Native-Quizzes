...
import AgricQuestion from "../components/data/agricData";
...
const AgricScreen = () => {
  ...
  const [selectedAnswers, setSelectedAnswers] = useState(
    Array(AgricQuestion.length).fill(null)
  );

  const handleAnswers = (selectedAns) => {
    const answer = AgricQuestion[presentQuest]?.answer;
    ...
    if (nextQuestion < AgricQuestion.length) {
  ...
  const totalQuestions = AgricQuestion.length;
    ...
    <View style={styles.container}>
      {showScore ? (
        ...
        <View>
          ...
          <Text style={styles.screen__title}>AgricQuestion Quiz</Text>
          <Text style={styles.questionNumber}>
            Question {presentQuest + 1} of {totalQuestions}
          </Text>
          <QuestionCard content={AgricQuestion[presentQuest]?.question} />
          <View style={styles.quest__opts}>
            {AgricQuestion[presentQuest]?.options.map((option) => {
              return (
                <TouchableOpacity
