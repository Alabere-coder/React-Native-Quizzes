...
import IRSQuestion from "../components/data/data1";
...
const IrsScreen = () => {
  ...
  const [selectedAnswers, setSelectedAnswers] = useState(
    Array(IRSQuestion.length).fill(null)
  );

  const handleAnswers = (selectedAns) => {
    const answer = IRSQuestion[presentQuest]?.answer;
    IRSQuestion;
    ...
    if (nextQuestion < IRSQuestion.length) {
  ...
  const totalQuestions = IRSQuestion.length;
    ...
    <View style={styles.container}>
      {showScore ? (
        ...
        <View>
          ...
          <Text style={styles.screen__title}>IRSQuestion Quiz</Text>
          <Text style={styles.questionNumber}>
            Question {presentQuest + 1} of {totalQuestions}
          </Text>
          <QuestionCard content={IRSQuestion[presentQuest]?.question} />
          <View style={styles.quest__opts}>
            {IRSQuestion[presentQuest]?.options.map((option) => {
              return (
                <TouchableOpacity
