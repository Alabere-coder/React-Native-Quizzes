...
import EconomicsQuestion from "../components/data/ecoData";
...
const Economics = () => {
  ...
  const [selectedAnswers, setSelectedAnswers] = useState(
    Array(EconomicsQuestion.length).fill(null)
  );

  const handleAnswers = (selectedAns) => {
    const answer = EconomicsQuestion[presentQuest]?.answer;
    ...
    if (nextQuestion < EconomicsQuestion.length) {
  ...
  const totalQuestions = EconomicsQuestion.length;
    ...
    <View style={styles.container}>
      {showScore ? (
        <ScrollView>
          <View style={styles.score__container}>
            ...
            <View style={styles.questionsContainer}>
              {EconomicsQuestion.map((item, index) => (
        ...
        <View>
          ...
          <Text style={styles.screen__title}>EconomicsQuestion Quiz</Text>
          <Text style={styles.questionNumber}>
            Question {presentQuest + 1} of {totalQuestions}
          </Text>
          <QuestionCard content={EconomicsQuestion[presentQuest]?.question} />
          <View style={styles.quest__opts}>
            {EconomicsQuestion[presentQuest]?.options.map((option) => {
              return (
                <TouchableOpacity
