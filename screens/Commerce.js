...
import CommerceQuestion from "../components/data/commerData";
...
const Commerce = () => {
  ...
  const [selectedAnswers, setSelectedAnswers] = useState(
    Array(CommerceQuestion.length).fill(null)
  );

  const handleAnswers = (selectedAns) => {
    const answer = CommerceQuestion[presentQuest]?.answer;
    ...
    if (nextQuestion < CommerceQuestion.length) {
  ...
  const totalQuestions = CommerceQuestion.length;
    ...
    <View style={styles.container}>
      {showScore ? (
        <ScrollView>
          <View style={styles.score__container}>
            ...
            <View style={styles.questionsContainer}>
              {CommerceQuestion.map((item, index) => (
        ...
        <ScrollView>
          ...
          <Text style={styles.screen__title}>CommerceQuestion Quiz</Text>
          <Text style={styles.questionNumber}>
            Question {presentQuest + 1} of {totalQuestions}
          </Text>
          <QuestionCard content={CommerceQuestion[presentQuest]?.question} />
          <View style={styles.quest__opts}>
            {CommerceQuestion[presentQuest]?.options.map((option) => {
              return (
                <TouchableOpacity
