...
import ChemistryQuestion from "../components/data/chemData";
...
const Chemistry = () => {
  ...
  const [selectedAnswers, setSelectedAnswers] = useState(
    Array(ChemistryQuestion.length).fill(null)
  );

  const handleAnswers = (selectedAns) => {
    const answer = ChemistryQuestion[presentQuest]?.answer;
    ...
    if (nextQuestion < ChemistryQuestion.length) {
  ...
  const totalQuestions = ChemistryQuestion.length;
    ...
    <View style={styles.container}>
      {showScore ? (
        <ScrollView>
          <View style={styles.score__container}>
            ...
            <View style={styles.questionsContainer}>
              {ChemistryQuestion.map((item, index) => (
        ...
        <View>
          ...
          <Text style={styles.screen__title}>ChemistryQuestion Quiz</Text>
          <Text style={styles.questionNumber}>
            Question {presentQuest + 1} of {totalQuestions}
          </Text>
          <QuestionCard content={ChemistryQuestion[presentQuest]?.question} />
          <View style={styles.quest__opts}>
            {ChemistryQuestion[presentQuest]?.options.map((option) => {
              return (
                <TouchableOpacity
