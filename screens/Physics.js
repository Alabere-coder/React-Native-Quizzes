...
import PhysicsQuestion from "../components/data/phyData";
...
const Physics = () => {
  ...
  const [selectedAnswers, setSelectedAnswers] = useState(
    Array(PhysicsQuestion.length).fill(null)
  );

  const handleAnswers = (selectedAns) => {
    const answer = PhysicsQuestion[presentQuest]?.answer;
    ...
    if (nextQuestion < PhysicsQuestion.length) {
  ...
  const totalQuestions = PhysicsQuestion.length;
    ...
    <View style={styles.container}>
      {showScore ? (
        <ScrollView>
          <View style={styles.score__container}>
            ...
            <View style={styles.questionsContainer}>
              {PhysicsQuestion.map((item, index) => (
        ...
        <View>
          ...
          <Text style={styles.screen__title}>PhysicsQuestion Quiz</Text>
          <Text style={styles.questionNumber}>
            Question {presentQuest + 1} of {totalQuestions}
          </Text>
          <QuestionCard content={PhysicsQuestion[presentQuest]?.question} />
          <View style={styles.quest__opts}>
            {PhysicsQuestion[presentQuest]?.options.map((option) => {
              return (
                <TouchableOpacity
