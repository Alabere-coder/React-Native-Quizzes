...
import ComputerQuestion from "../components/data/compData";
...
const ComputerScreen = () => {
  ...
  const [selectedAnswers, setSelectedAnswers] = useState(
    Array(ComputerQuestion.length).fill(null)
  );

  const handleAnswers = (selectedAns) => {
    const answer = ComputerQuestion[presentQuest]?.answer;
    ...
    if (nextQuestion < ComputerQuestion.length) {
  ...
  const totalQuestions = ComputerQuestion.length;
    ...
    <View style={styles.container}>
      {showScore ? (
        ...
        <View>
          ...
          <Text style={styles.screen__title}>ComputerQuestion Quiz</Text>
          <Text style={styles.questionNumber}>
            Question {presentQuest + 1} of {totalQuestions}
          </Text>
          <QuestionCard content={ComputerQuestion[presentQuest]?.question} />
          <View style={styles.quest__opts}>
            {ComputerQuestion[presentQuest]?.options.map((option) => {
              return (
                <TouchableOpacity
