...
import English from "../components/data/engData";
...
const EnglishScreen = () => {
  ...
  const [selectedAnswers, setSelectedAnswers] = useState(
    Array(English.length).fill(null)
  );

  const handleAnswers = (selectedAns) => {
    const answer = English[presentQuest]?.answer;
    ...
    if (nextQuestion < English.length) {
  ...
  const totalQuestions = English.length;
    ...
    <View style={styles.container}>
      {showScore ? (
        ...
        <View>
          ...
          <Text style={styles.screen__title}>English Quiz</Text>
          <Text style={styles.questionNumber}>
            Question {presentQuest + 1} of {totalQuestions}
          </Text>
          <QuestionCard content={English[presentQuest]?.question} />
          <View style={styles.quest__opts}>
            {English[presentQuest]?.options.map((option) => {
              return (
                <TouchableOpacity
