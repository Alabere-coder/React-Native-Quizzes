...
import Mathematics from "../components/data/mathData";
...
const MathScreen = () => {
  ...
  const [selectedAnswers, setSelectedAnswers] = useState(
    Array(Mathematics.length).fill(null)
  );

  const handleAnswers = (selectedAns) => {
    const answer = Mathematics[presentQuest]?.answer;
    ...
    if (nextQuestion < Mathematics.length) {
  ...
  const totalQuestions = Mathematics.length;
    ...
    <View style={styles.container}>
      {showScore ? (
        ...
        <View>
          ...
          <Text style={styles.screen__title}>Mathematics Quiz</Text>
          <Text style={styles.questionNumber}>
            Question {presentQuest + 1} of {totalQuestions}
          </Text>
          <QuestionCard content={Mathematics[presentQuest]?.question} />
          <View style={styles.quest__opts}>
            {Mathematics[presentQuest]?.options.map((option) => {
              return (
                <TouchableOpacity
