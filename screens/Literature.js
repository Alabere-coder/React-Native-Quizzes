...
import LiteratureQuestion from "../components/data/literatureData";
...
const LiteratureScreen = () => {
  ...
  const [selectedAnswers, setSelectedAnswers] = useState(
    Array(LiteratureQuestion.length).fill(null)
  );

  const handleAnswers = (selectedAns) => {
    const answer = LiteratureQuestion[presentQuest]?.answer;
    ...
    if (nextQuestion < LiteratureQuestion.length) {
  ...
  const totalQuestions = LiteratureQuestion.length;
    ...
    <View style={styles.container}>
      {showScore ? (
        <ScrollView>
          <View style={styles.score__container}>
            ...
            <View style={styles.questionsContainer}>
              {LiteratureQuestion.map((item, index) => (
        ...
        <View>
          ...
          <Text style={styles.screen__title}>LiteratureQuestion Quiz</Text>
          <Text style={styles.questionNumber}>
            Question {presentQuest + 1} of {totalQuestions}
          </Text>
          <QuestionCard content={LiteratureQuestion[presentQuest]?.question} />
          <View style={styles.quest__opts}>
            {LiteratureQuestion[presentQuest]?.options.map((option) => {
              return (
                <TouchableOpacity
